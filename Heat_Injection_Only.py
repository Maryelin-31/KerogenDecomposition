...
def viscosity(fid, time, step):
    ...
    plot = plt.contourf(Vis, 20, extent=[xx.min(), xx.max(), yy.min(), yy.max()], cmap='coolwarm', vmin=0, vmax=0.5)
    ...
    if fid == fid_w:
        fid = 'water'
    plt.title(f'Viscosity of {fid} (Kg) at {Time} year ', fontsize=10)
    plt.xlabel('x/m')
    plt.ylabel('y/m')
...
def Save(path, step, time):
    ...
    with open(SavePath, 'w') as file:
        ...
        for cell in model.cells:
            ...
            #     continue
            # else:
            file.write(f'{step} {Time} {x} {y} {z} {cell.get_fluid(fid_k).mass} {cell.get_fluid(fid_o).mass} {cell.get_fluid(fid_k).get_attr(fa_t)} {cell.get_attr(ca_t)}\n')
...
def run_2():
    ...
    if os.path.exists(f'production_{name}_{temp_inje}'):
        import shutil
        shutil.rmtree(f'productione_{name}_{temp_inje}')
    time = 0
    dt = 1.0e-8
