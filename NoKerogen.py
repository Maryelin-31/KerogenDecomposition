...
def add_cell(pos, vol, pre, temp, sat):
    ...
    cell.set_attr(ca_mc, vol*2600*1000).set_attr(ca_t, temp).set_attr(ca_g, 100)
    cell.fluid_number = 4
...
def saturation(fid, time, step):
    ...
    plt.title(f'Saturation of {fid} (Kg) at {Time} year ', fontsize=10)
    plt.xlabel('x/m')
    plt.ylabel('y/m')
...
def Pressure(time,step): #Matrix 
    ...
    for cell in model.cells:
        ...
        X.append(x)
        Y.append(y)
        if cell.pos == cell_1.pos:
            Presu.append(cell1.pre)
        Presu.append(cell.pre)
    pre = Presu[:len(Presu) - 2]
    [xx, yy] = np.meshgrid(X, Y)
    fig, ax = plt.subplots()
    pre = np.array(pre)
    Pre= np.transpose(pre.reshape((pre.size // 25, 25)))
    pini = np.min(Pre)
    pmax = np.max(Pre)
    plot = plt.contourf(Pre, 20, extent=[xx.min(), xx.max(), yy.min(), yy.max()], cmap='coolwarm', vmin= pini, vmax= pmax)
    Time = round(time / (3600*24*365), 1)
    plt.title(f'Pressure of Rock (Pa) at {Time} year', fontsize=10)
    plt.xlabel('x/m')
    plt.ylabel('y/m')
    plt.clim(vmin= pini, vmax= pmax)
    fig.colorbar(ScalarMappable(norm=plot.norm, cmap=plot.cmap))
    
    # PlotPath = f'Pressure_{step}.png'   
    # folder = os.path.join(os.getcwd(),f'Pressure_cell_Plot_{name}')
    # plotfile = os.path.join(folder, PlotPath)
    # plt.savefig(plotfile , format='png')
    plt.show()
    # plt.close()
...
def Save(path, step, time):
    ...
    Savefolder = os.path.join(os.getcwd(), f'Model_Results_{name}')
...
def run_2():    
    name = os.path.basename(__file__)
    # if os.path.exists(f'Model_Results_{name}'):
    #     import shutil
    #     shutil.rmtree(f'Model_Results_{name}')
    # os.mkdir(f'Model_Results_{name}')
    # if os.path.exists(f'Pressure_cell_Plot_{name}'):
    #     import shutil
    #     shutil.rmtree(f'Pressure_cell_Plot_{name}')
    # os.mkdir(f'Pressure_cell_Plot_{name}')
    ...
    if os.path.exists(f'production_{name}'):
        import shutil
        shutil.rmtree(f'production_{name}')
    time = 0
    dt = 1.0e-8
    with open(f'production_{name}.txt', 'w') as f:
        for step in range(2000):
            ...
            if step % 1 == 0:
                ...
                f.write(f'{step} {round(time/(3600*24*365),1)} {cell1.get_fluid(fid_o).mass - mass_ini} {cell1.get_fluid(fid_o).vol - volume_ini} {cell_1.get_fluid(fid_o).vis}\n')
